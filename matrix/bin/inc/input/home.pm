...
###########################################
    ...
    say LOGDIE "No configuration file found (searched ", 
           join (", ", @{ $self->{conf_paths} }), 
           ")" unless defined $self->{conf};
 
    if(ref( $self->{conf} ) ne "HASH") {
        say LOGDIE "Configuration file invalid (not a hash)";
    }
...
###########################################
    ...
    dbmopen(%{$self->{dbm}},
        ...
        say LOGDIE "Cannot open $self->{db_file}";
...
###########################################
    ...
    if(! exists $self->{receivers}->{$receiver}) {
        say ERROR "Unknown receiver '$receiver'";
        return undef;
    }
 ...
 
    {
      ...
      say DEBUG "Addressing HC=$house_code UC=$unit_code";
      $send->($self->{serial}, $house_code . $unit_code);
   
      say DEBUG "Sending command $cmd $self->{commands}->{$cmd}";
      $send->($self->{serial},
                    $house_code .
                    $self->{commands}->{$cmd});
    }
    ...
    if(defined $self->{db_file}) {
        say DEBUG "Setting db status of $receiver to $cmd";
...
###########################################
    ...
    open my $fh, ">>$self->{lockfile}" or
        say LOGDIE "Cannot open lockfile $self->{lockfile} ($!)";
    flock($fh, LOCK_EX);
...
###########################################
    ...
    if(! defined $self->{fh}) {
        say LOGDIE "Called unlock without previous lock";
    }
